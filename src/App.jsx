...
async function copyContactList() {
  ...
  function getContactsForSelectedMobileGroups() {
    const seenIds = new Set()
    return selectedMobileTextGroupIds
      .flatMap((groupId) => getContactsForGroup(groupId))
      .filter((contact) => {
  ...
  async function sendTextToSelectedGroups() {
    if (!selectedMobileTextGroupIds.length) {
      showError('Select at least one text group first.')
      return
    }
      ...
      {activeTab === 'mobile' && (
        <div style={styles.singleColumnWrap}>
          <div style={styles.sectionCard}>
            ...
            <div style={styles.mobileGroupPickerGrid}>
              <div style={styles.mobileGroupChecklistBox}>
                <div style={styles.mobileGroupChecklistTitle}>Select Group(s)</div>
                {contactGroups.length === 0 ? (
                  <div style={styles.smallText}>No contact groups saved yet.</div>
                ) : (
                  contactGroups.map((group) => (
                    <label key={group.id} style={styles.mobileGroupCheckRow}>
                      <input
                        type="checkbox"
                        checked={selectedMobileTextGroupIds.includes(group.id)}
                        onChange={() => toggleMobileTextGroup(group.id)}
                      />
              ...
              <div style={styles.mobileSelectedContactsBox}>
...
const styles = {
  ...
  mobileSelectedContactNames: {
    display: 'grid',
    gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))',
    gap: 8,
  },
  mobileSelectedContactName: {
    padding: '7px 10px',
...
,
  ...
  smallDangerButton: {
    ...
    padding: '6px 10px',
    cursor: 'pointer',
    fontSize: '12px',import React, { useEffect, useMemo, useState } from 'react'
import { supabase } from './lib/supabase'
